...
t_ref = jnp.array(traj_data['t'])
x_ref = jnp.array(traj_data['x'])
...
n = system.state_dim
if start_locally:
    e0 = jax.random.uniform(key0, (num_traj, n), minval=e_min, maxval=e_max)
    x0 = x_ref[:num_traj, 0, :] + e0
else:
    x0 = jax.random.uniform(key0, (num_traj, n), minval=x_min, maxval=x_max)
...
for name, ctrl in progress_bar:
    ...
    x, u, xr, ur, J = jax.vmap(simulate_ctrl, in_axes)(
        x0, t, t_ref[:, :-1], x_ref[:, :-1], u_ref, u_lb, u_ub, Q, R
    )
